...
from apps.accounts.serializers import CandidateProfileSerializer, CompanySerializer
    ...
    def validate_experience_required(self, value):
        ...
        if value > 45:
            raise serializers.ValidationError(
                "Please enter a valid experience range (0-45)."
            )
        return value
...
class FullCandidateDetailSerializer(serializers.ModelSerializer):
    ...
    location = serializers.CharField(
        source="applicant.current_location", read_only=True
    )
    experience = serializers.FloatField(
        source="applicant.total_experience", read_only=True
    )
...
class ApplicationStatusUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate_status(self, value):
        valid_statuses = [
            choice[0] for choice in JobApplication._meta.get_field("status").choices
        ]
...
class HRApprovalSerializer(serializers.ModelSerializer):
    ...
    applicant_email = serializers.EmailField(
        source="applicant.user.email", read_only=True
    )
