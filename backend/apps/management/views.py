...
class AllCompanyEmployeesView(views.APIView):
    ...
    def get(self, request, version):
        ...
        employees = HRProfile.objects.filter(company=hr_profile.company).exclude(
            user=request.user
        )
...
class ToggleEmployeesView(views.APIView):
    ...
    def patch(self, request, version, id=None):
        try:
            ...
            return Response(
                {"error": "Hr or Interviewer does not exist"},
        ...
        if toggeld_user.company == hr_profile.company:
            ...
            return Response(
                {"error": "This hr not belongs to your firm"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class ToggleEmployeeRoleView(views.APIView):
    ...
    def patch(self, request, version, id=None):
        ...
        if serializer.is_valid():
            ...
            return Response(
                {"success": "Role updated successfully", "role": toggled_user.role},
...
class GetCandidateView(views.APIView):
    ...
    def get(self, request, version):
        try:
            ...
            return Response(
                {"error": "User is guest"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class CandidateDashboardView(APIView):
    ...
    def get(self, request, version):
        ...
        return Response({"metrics": stats, "recent_applications": app_list})
...
class HRDashboardView(APIView):
    ...
    def get(self, request, version):
        ...
        stats = {
            ...
            "pending_reviews": JobApplication.objects.filter(
                job__company=company, status="APPLIED"
            ).count(),
            "hires_to_date": hr_profile.hires_count,
        }

        hot_jobs = jobs.annotate(app_count=Count("applications")).order_by(
            "-app_count"
        )[:3]
        hot_jobs_list = [
            {"title": j.title, "count": j.app_count, "is_active": j.is_active}
            for j in hot_jobs
        ]
    ...
    def get(self, request, version):
        ...
        upcoming_interviews = (
            JobApplication.objects.filter(
                interviewer=user, status="SCHEDULED", scheduled_at__gte=timezone.now()
            )
        ...
        stats = {
            ...
            "completed_this_month": JobApplication.objects.filter(
                interviewer=user, applied_at__month=timezone.now().month
            )
        ...
        return Response({"metrics": stats, "schedule": schedule})
